...
Vector Row Count: ${result.data.vector.row_count}
SQL Row Count: ${result.data.sql.row_count}
Answer Summary: ${result.answer.substring(0, 500)}
    `;
    await writeFile("final_verify.log", output);
  } catch (error: any) {
    await writeFile("final_verify.log", "Error: " + error.message);
  } finally {
    await db.close();
  }
}
